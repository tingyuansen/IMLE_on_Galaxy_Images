...
#=============================================================================================================
            ...
            ind_shuffle = torch.from_numpy(ind_shuffle).cuda()
...
#-----------------------------------------------------------------------------------------------------------
            # save the mock sample
            if (epoch+1) % staleness == 0:
                np.savez("../results_2D_zdim=4_random_batch_cont_times1_epoch=" + str(epoch) + ".npz",\
                ...
                np.savez("../results_2D_random_zdim=4_random_batch_cont_times1_epoch=" + str(epoch) + ".npz",\
                          samples_np=samples_random, mse_err=err / num_batches)

                # save network
                torch.save(self.model.state_dict(), '../net_weights_2D_zdim=4_random_batch_cont_times1_epoch=' \
                             + str(epoch) + '.pth')
