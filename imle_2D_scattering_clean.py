...
#-----------------------------------------------------------------------------------------------------------
            # save the mock sample
            if (epoch+1) % staleness == 0:
                np.savez("../results_2D_j=2_channel=64_decay.npz", data_np=data_np, z_Sx_np=z_Sx.cpu().data.numpy(),\
                                samples_np=samples_predict)

                # make random mock
                samples_random = self.model(z_Sx_all[:10**3]).cpu().data.numpy()
                np.savez("../results_2D_random_j=2_channel=64_decay.npz", samples_np=samples_random)
...
#---------------------------------------------------------------------------------------------
    ...
    torch.save(imle.model.state_dict(), '../net_weights_2D_j=2_channel=64_decay.pth')
