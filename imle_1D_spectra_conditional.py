...
#-----------------------------------------------------------------------------------------------------------
            ...
            if (epoch+1) % staleness == 0:
                ...
                # save closet models
                np.savez("../results_spectra_deconv_256x4_no_decay" + str(epoch) +  ".npz", data_np=data_np,\
                                               z_Sx_np=z_Sx.cpu().data.numpy(),\
                                               samples_np=samples_predict)

                np.savez("../mse_err_deconv_256x4_no_decay_" + str(epoch) +  ".npz",\
                                                mse_err=err/num_batches)

                # save network
                torch.save(self.model.state_dict(), '../net_weights_spectra_deconv_256x4_no_decay_epoch=' + str(epoch) + '.pth')
